import XCTest
@testable import MarinaCompanion

final class MarinaServiceClientTests: XCTestCase {
    func testDecodesServiceStatus() throws {
        let payload = ##"{"ok":true,"data":{"projects":[{"id":"prj_1","name":"Marina","color":"#338CFF","servers":[{"id":"srv_1","name":"website","port":3000,"state":"running","healthy":true,"pid":42}]}]}}"##
        let envelope = try JSONDecoder().decode(MarinaStatusEnvelope.self, from: Data(payload.utf8))

        XCTAssertTrue(envelope.ok)
        XCTAssertEqual(envelope.data.projects.first?.name, "Marina")
