...
final class CompanionModel: ObservableObject {
    ...
    func refresh() async {
        ...
        do {
            let status = try await MarinaServiceClient.status()
    ...
    func perform(_ action: String, on server: CompanionServer) async {
        ...
        do {
            try await MarinaServiceClient.perform(action, serverID: server.id)
...
private struct CompanionView: View {
    ...
    var body: some View {
        NavigationStack {
            Group {
                ...
                case .connecting:
                    ProgressView("Connecting to Marina…")
    ...
    private var emptyProjects: some View {
        VStack(spacing: 12) {
            Image(systemName: "shippingbox")
                .font(.system(size: 30))
                .foregroundStyle(.secondary)
            Text("No Marina projects")
                .font(.headline)
            Text("Add a project in Marina, or explore the demo to try every control.")
                .font(.callout)
                .foregroundStyle(.secondary)
            Button("Explore Demo") {
                model.enterDemoMode()
    ...
    private func unavailableView(_ message: String) -> some View {
        VStack(spacing: 16) {
            ...
            Text("Marina service is not running")
                .font(.title2.bold())
            Text("Connect to the local Marina service, or explore every feature in Demo Mode.\n\(message)")
