...
enum MemoryDiagnosticsEngine {
        ...
        for process in heavyExternal {
            diagnostics.append(
                MemoryDiagnostic(
                    ...
                    title: "\(process.displayName) is heavy outside Marina",
                    summary: "PID \(process.pid) owns \(bytes(process.memoryBytes)) and uses \(bytes(process.residentMemoryBytes)) of RAM. Marina will not stop it automatically.",
        ...
        if diagnostics.isEmpty {
            return [
                MemoryDiagnostic(
                    ...
                    fix: "Keep this dashboard open while reproducing a slowdown. Marina will surface a new recommendation as soon as the pattern becomes significant.",
                    serverID: nil,
                    processID: nil
                ),
            ]
    ...
    private static func duplicateDevProcessDiagnostics(
        ...
        return Dictionary(grouping: candidates, by: { $0.0 }).compactMap { family, entries in
            ...
            return MemoryDiagnostic(
                id: "duplicate-\(family)",
                severity: .notice,
                title: "Multiple \(family) sessions run outside Marina",
                summary: "\(entries.count) related processes own \(bytes(total)) in total (PIDs \(pids)). Some may be stale or duplicate dev servers.",
                fix: "Close stale sessions, then register each persistent dev server in Marina so its full process tree can be stopped and monitored safely.",
                serverID: nil,
    ...
    static func advice(for command: String, managed: Bool) -> String {
        ...
        if managed {
            ...
            prefix = "Stop the parent dev session, not only this child, then move it under Marina if it should stay running. "
        } else {
