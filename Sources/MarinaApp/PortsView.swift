...
struct PortsView: View {
    ...
    var body: some View {
        ...
        .confirmationDialog(
            ...
            Text(
                "Marina revalidates the listener before stopping it. Regular processes receive SIGTERM; "
...
struct ActivePortGroup: Identifiable {
    ...
    var summary: String {
        ...
        let system = ports.filter { $0.kind == .system || $0.kind == .marina }.count
    ...
    func stop(_ port: ActivePort, using supervisor: Supervisor) async {
        ...
        case .managed:
            guard let serverID = port.serverID, let runtime = supervisor.runtime(for: serverID) else {
                actionError = "Marina could not find the managed server for port \(port.port)."
                return
            }
            runtime.stop()
        case .external:
