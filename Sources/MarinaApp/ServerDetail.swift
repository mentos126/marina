...
struct ServerDetail: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if let conflict, !conflict.ownedByMarina {
                VStack(spacing: 0) {
            ...
            if let metrics = runtime.processMetrics, showsResources {
                HStack(spacing: 18) {
                    ...
                    compactResource(
                        ...
                        color: .blue,
                        label: "Processes",
                        help: "Processes Marina groups together for this server"
                    )
    ...
    private func conflictBanner(_ occupant: PortOccupant) -> some View {
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(occupant.dockerContainerID == nil ? "Running outside Marina" : "Docker container outside Marina")
            ...
            .controlSize(.small)
            .help("Stop the current owner safely, then start \(runtime.projectName) / \(runtime.config.name) under Marina")
        }
