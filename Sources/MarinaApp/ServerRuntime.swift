...
final class ServerRuntime: NSObject, ObservableObject, LocalProcessDelegate, TerminalViewDelegate {
    ...
    private var settings: MarinaConfig
    ...
    init(config: ServerConfig, project: Project, settings: MarinaConfig) {
        self.id = config.id
        self.config = config
    ...
    /// generic system console. The fallbacks keep Marina usable on another Mac.
    ...
    func apply(config: ServerConfig, project: Project, settings: MarinaConfig) {
    ...
    func restartForMemoryLimit(projectFootprintBytes: UInt64, limitBytes: UInt64) {
        ...
        logs.note(message)
        terminal?.feed(text: "\r\n\u{1B}[33m[marina] \(message)\u{1B}[0m\r\n")
        restart()
    }
    ...
    private func spawn() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            view.feed(text: "\u{1B}[2m[marina] \(self.config.command)\u{1B}[0m\r\n")
            proc.startProcess(
                executable: "/bin/zsh",
    ...
    private func environmentArray() -> [String] {
        ...
        // Marina owns a real PTY. Do not inherit NO_COLOR from the app launcher
        ...
        env["TERM_PROGRAM"] = "Marina"
        env["MARINA"] = "1"
        env["MARINA_SERVER"] = config.name
    ...
    private func scheduleTemporaryTimeout() {
        ...
        let work = DispatchWorkItem { [weak self] in
            ...
            self.terminal?.feed(text: "\r\n\u{1B}[31m[marina] \(self.lastError!)\u{1B}[0m\r\n")
            self.stop()
        }
        timeoutWork = work
    ...
    /// Stop a listener launched outside Marina, then start this configured
    /// server as soon as the port is released. The explicit UI/CLI action is
    /// the authority boundary; Marina never takes over automatically.
    @discardableResult
    func takeOverPort() -> Bool {
        ...
        setState(.starting)
        terminal?.feed(text: "\u{1B}[33m[marina] identifying the owner of port \(port)…\u{1B}[0m\r\n")
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            DispatchQueue.main.async {
                ...
                case .success(let outcome):
                    ...
                    self.terminal?.feed(
                        text: "\u{1B}[33m[marina] stopped \(outcome.description); starting the configured server…\u{1B}[0m\r\n"
                    )
    ...
    func processTerminated(_ source: LocalProcess, exitCode: Int32?) {
        DispatchQueue.main.async { [weak self] in
            ...
            self.terminal?.feed(text: "\r\n\u{1B}[2m[marina] exited (\(code))\u{1B}[0m\r\n")
