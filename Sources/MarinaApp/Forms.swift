...
struct ServerForm: View {
    ...
    private var automaticSetup: some View {
        if isDetecting {
            ...
            Section("No commands detected") {
                Label {
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Marina could not find a development command")
            ...
            Section {
                ...
                Text("Choose what Marina should run for \(projectName). You can edit every setting later.")
                    .font(.system(size: 11))
                    .foregroundStyle(.secondary)
            }
        }
    ...
    private var portHelpText: String {
        if let port = Int(portText.trimmingCharacters(in: .whitespaces)) {
            return "Marina will open http://localhost:\(port) and use this port for health checks."
        }
