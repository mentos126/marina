...
/// drives through the `marina` CLI.
///
/// Loopback keeps the API off the network but does not authorize a caller: the
/// user's browser is on loopback too. Every request goes through
/// `ControlRequestGuard` before it is routed.
final class ControlServer {
    /// Largest request Marina will buffer. The bodies here are small JSON
    /// objects; anything larger is a caller trying to grow this process.
    private static let maxRequestBytes = 1 << 20
    /// A request that never completes its body must not hold a connection open.
    private static let requestTimeout: TimeInterval = 15

    private var listener: NWListener?
    private let queue = DispatchQueue(label: "dev.marina.api")
    private let supervisor: Supervisor
    private(set) var port: Int
    private var requestGuard: ControlRequestGuard?
    ...
    func start() {
        stop()
        // Writes ~/.config/marina/token at 0600 on first launch.
        requestGuard = ControlRequestGuard(port: port, token: ControlToken.loadOrCreate())
        ...
        do {
            ...
            listener.stateUpdateHandler = { state in
                if case .failed(let error) = state {
                    NSLog("[marina] control API failed: \(error)")
                }
            }
            listener.start(queue: queue)
            self.listener = listener
        } catch {
            NSLog("[marina] could not bind control API on \(port): \(error)")
        }
    }
    ...
    private func receive(_ connection: NWConnection, buffer: Data) {
        connection.receive(minimumIncompleteLength: 1, maximumLength: 65_536) { [weak self] data, _, isComplete, error in
            ...
            let declared = HTTPRequest.declaredContentLength(in: buffer) ?? 0
            if buffer.count > Self.maxRequestBytes || declared > Self.maxRequestBytes {
                self.send(status: 413, message: "Request larger than 1 MB", on: connection)
                return
            }
    ...
    private func respond(to request: HTTPRequest, on connection: NWConnection) {
        if let rejection = requestGuard?.rejection(for: request) {
            send(status: rejection.status, message: rejection.message, on: connection)
            return
        }
    ...
    private func send(status: Int, message: String, on connection: NWConnection) {
        send(status: status, body: (try? fail(message)) ?? Data(), on: connection)
    }
    ...
    private func send(status: Int, body: Data, on connection: NWConnection) {
        ...
        // No Access-Control-Allow-Origin, ever: a browser must not read this.
    ...
    private func route(_ request: HTTPRequest) -> (Int, Data) {
        do {
            ...
            case ("POST", "/start"), ("POST", "/stop"), ("POST", "/restart"):
                let body: MarinaAPI.TargetRequest = try request.decode()
            ...
            case ("GET", "/logs"):
                ...
                return (200, try ok(MarinaAPI.LogsResponse(server: runtime.config.name, lines: runtime.logTail(tail))))
            ...
            case ("POST", "/projects/add"):
                let body: MarinaAPI.AddProjectRequest = try request.decode()
            ...
            case ("POST", "/projects/remove"):
                let body: MarinaAPI.RemoveRequest = try request.decode()
                ...
                return (200, try ok(MarinaAPI.ActionResponse(affected: [project.id], message: "Removed project \(project.name)")))

            case ("POST", "/servers/add"):
                let body: MarinaAPI.AddServerRequest = try request.decode()
            ...
            case ("POST", "/temporary/run"):
                let body: MarinaAPI.RunTemporaryRequest = try request.decode()
            ...
            case ("POST", "/actions/run"):
                let body: MarinaAPI.RunServerActionRequest = try request.decode()
            ...
            case ("POST", "/memory-limit"):
                let body: MarinaAPI.UpdateMemoryLimitRequest = try request.decode()
                if let query = body.project {
                    ...
                    let value = body.mode == .custom
                        ...
                        : body.mode.rawValue
                    return (200, try ok(MarinaAPI.ActionResponse(
                        affected: [project.id],
            ...
            case ("POST", "/servers/update"):
                let body: MarinaAPI.UpdateServerRequest = try request.decode()
            ...
            case ("POST", "/servers/remove"):
                ...
                return (200, try ok(MarinaAPI.ActionResponse(affected: [runtime.id], message: "Removed server \(name)")))

            case ("POST", "/servers/take-over"):
                let body: MarinaAPI.TakeOverRequest = try request.decode()
                guard let runtime = supervisor.resolveServer(body.server) else {
                    return (404, try fail("No server matching '\(body.server)'"))
                }
                ...
                return (200, try ok(MarinaAPI.ActionResponse(
                    affected: [runtime.id],
                    message: "Moving port \(runtime.config.port.map(String.init) ?? "") to Marina"
                )))
            ...
            case ("GET", "/ports"):
                ...
                return (200, try ok(MarinaAPI.PortQueryResponse(port: port, occupant: supervisor.occupant(of: port))))

            case ("POST", "/ports/kill"):
                let body: MarinaAPI.KillPortRequest = try request.decode()
            ...
            case ("POST", "/open"):
                let body: MarinaAPI.OpenRequest = try request.decode()
                ...
                return (200, try ok(MarinaAPI.ActionResponse(affected: [], message: "Opened Marina\(destination)")))

            case ("POST", "/quit"):
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    NSApplication.shared.terminate(nil)
                }
                return (200, try ok(MarinaAPI.ActionResponse(affected: [], message: "Quitting Marina")))
    ...
    private func act(_ path: String, _ target: MarinaAPI.TargetRequest) throws -> MarinaAPI.ActionResponse {
        ...
        if let query = target.server {
            ...
            return MarinaAPI.ActionResponse(affected: [runtime.id], message: "\(verb) \(runtime.config.name)")
        }
        ...
        if let query = target.project {
            ...
            return MarinaAPI.ActionResponse(
        ...
        return MarinaAPI.ActionResponse(affected: all.map(\.id), message: "\(verb) all servers")
    }
    ...
    private func ok<T: Codable>(_ value: T) throws -> Data {
        try MarinaAPI.encoder().encode(MarinaAPI.Envelope(ok: true, data: value))
    }

    private func fail(_ message: String) throws -> Data {
        try MarinaAPI.encoder().encode(MarinaAPI.Envelope<MarinaAPI.Empty>(ok: false, data: nil, error: message))
    }
}
...
struct HTTPRequest {
    ...
    /// Header names lowercased. A repeated header keeps its first value, so a
    /// smuggled second copy cannot override the one that was validated.
    ...
    init?(_ raw: Data) {
        ...
        for line in lines.dropFirst() {
            ...
            if parsedHeaders[name] == nil { parsedHeaders[name] = value }
        }
        headers = parsedHeaders

        let contentLength = max(0, Int(parsedHeaders["content-length"] ?? "") ?? 0)
    ...
    func decode<T: Codable>() throws -> T {
        ...
        do {
            return try MarinaAPI.decoder().decode(T.self, from: body)
        } catch {
            throw DecodeError(message: "Invalid JSON body: \(error.localizedDescription)")
        }
    }

    /// Content-Length as announced, readable as soon as the headers are in, so an
    /// oversized body is refused before it is buffered.
    static func declaredContentLength(in raw: Data) -> Int? {
        guard let headerEnd = HTTPRequest.range(of: Data("\r\n\r\n".utf8), in: raw) else { return nil }
        guard let text = String(data: raw.subdata(in: 0..<headerEnd.lowerBound), encoding: .utf8) else { return nil }
