...
struct ResourceDashboard: View {
    ...
    private func externalProcessDetails(_ process: ExternalProcessSnapshot) -> some View {
        VStack(alignment: .leading, spacing: 7) {
            HStack(spacing: 8) {
                Label("Outside Marina", systemImage: "arrow.up.right.square")
                Spacer()
    ...
    private func externalStopMessage(_ process: ExternalProcessSnapshot) -> String {
        ...
        return "Marina will revalidate the command, then send SIGTERM to \(scope). It never sends SIGKILL automatically. Target: \(process.terminationCommand)"
    }
    ...
    private var outsideMarinaProcesses: some View {
        dashboardSection(
            title: "Top processes outside Marina",
            ...
            .accessibilityLabel("Processes outside Marina")
        }
    }
