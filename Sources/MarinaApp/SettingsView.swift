...
private struct GeneralSettingsView: View {
    ...
    @AppStorage(MarinaPreferences.showMenuBarNameKey) private var showMenuBarName = false
    ...
    var body: some View {
        Form {
            Section("Menu bar") {
                Toggle("Show Marina in the menu bar", isOn: menuBarBinding)
                ...
                Text("Hold Command and drag Marina to reposition it in the menu bar.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }

            Section("Dock") {
                Toggle("Show Marina in the Dock", isOn: dockBinding)

                Text("When hidden, Marina stays in the menu bar and keeps supervising servers.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }

            Section("Startup") {
                Toggle("Launch Marina at login", isOn: Binding(
                    get: { launchAtLogin },
                    set: updateLaunchAtLogin
                ))

                Text("Closing the main window keeps Marina and its managed servers running.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            ...
            Section("Configuration") {
                LabeledContent("Config file") {
                    Text(NSString(string: MarinaPaths.configFile.path).abbreviatingWithTildeInPath)
                        .font(.system(size: 11, design: .monospaced))
                        .foregroundStyle(.secondary)
                        .textSelection(.enabled)
                }
                Button("Reveal Config File") {
                    NSWorkspace.shared.activateFileViewerSelecting([MarinaPaths.configFile])
                }
            }

            Section("Privacy") {
                Text("Marina runs entirely on this Mac.")
                Text(
                    "It sends no analytics, checks for no updates, and makes no request of its own beyond 127.0.0.1. Nothing about your projects, commands, environments, or logs leaves this machine."
                )
                .font(.caption)
                .foregroundStyle(.secondary)
            }
    ...
    var body: some View {
        Form {
            Section("Health checks") {
                ...
                Text("Marina restarts unhealthy servers only when automatic restart is enabled for that server.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        ...
        Form {
            Section {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text("Marina measures each project's total footprint every two seconds. Three consecutive samples above its limit restart every running server in that project, then sampling starts fresh on the new processes.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
