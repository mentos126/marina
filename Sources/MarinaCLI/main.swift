...
func client(_ options: GlobalOptions) -> MarinaClient {
    MarinaClient(port: options.apiPort)
}

func emit<T: Codable>(_ value: T, json: Bool, human: (T) -> String) {
    if json {
        let data = (try? MarinaAPI.encoder().encode(value)) ?? Data()
...
private func namedServers(in status: MarinaStatus) -> [NamedServerStatus] {
...
func renderCompact(_ status: MarinaStatus) -> String {
    let servers = namedServers(in: status)
    guard !servers.isEmpty else {
        return "No servers configured. Use 'marina temp' for one-off work, or add a project for long-lived services."
    }
...
func renderDetailed(_ status: MarinaStatus) -> String {
    guard !status.projects.isEmpty || !status.temporaryServers.isEmpty else {
        return "Nothing running yet. Use 'marina temp' for small one-off work, or add a project for long-lived services."
    }
    var out: [String] = []
...
struct Marina: ParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "marina",
        abstract: "Control Marina, the macOS dev server manager.",
        discussion: """
        Marina keeps local dev servers running on fixed ports, restarts them when \
        they crash, and exposes everything through this CLI so an agent can drive it.

        Every command starts Marina.app if it is not already running.
        """,
...
struct Status: ParsableCommand {
    ...
    func run() throws {
        do {
            let status = try client(options).get("status", as: MarinaStatus.self)
...
private func runAction(_ path: String, _ target: TargetOptions, _ options: GlobalOptions) {
    let body = MarinaAPI.TargetRequest(server: target.server, project: target.project)
    do {
        let response = try client(options).post(path, body, as: MarinaAPI.ActionResponse.self)
...
struct Stop: ParsableCommand {
    ...
    @Flag(name: .long, help: "Stop every server Marina manages.")
    var all = false
...
struct Action: ParsableCommand {
    ...
    func run() throws {
        ...
        let body = MarinaAPI.RunServerActionRequest(
            server: server,
            action: action,
...
struct Logs: ParsableCommand {
    ...
    func run() throws {
        ...
        do {
            let response = try client(options).get("logs?server=\(escaped)&tail=\(tail)", as: MarinaAPI.LogsResponse.self)
...
struct Temp: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: "The command starts in the background, returns a job ID immediately, and is stopped with its entire process group at the timeout. Use 'marina wait <id>' to collect the result.",
    ...
    @Option(name: .long, help: "Short label shown in Marina. Defaults to the command.")
    var name: String?
    ...
    func run() throws {
        ...
        guard let selectedCommand = (command ?? commandOption)?.trimmingCharacters(in: .whitespacesAndNewlines),
            ...
            fail("Missing command. Example: marina temp 'npm run build'")
        }
        guard let timeoutSeconds = TemporaryTimeout.parse(timeout) else {
            fail("Bad --timeout '\(timeout)'. Use 30s, 10m, 2h, or seconds up to 7 days")
        }
        ...
        let body = MarinaAPI.RunTemporaryRequest(
            name: selectedName,
...
struct Wait: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: "The job keeps running under Marina if this command is interrupted. Completed job metadata remains available for one hour."
    )

    @Argument(help: "Temporary job ID returned by 'marina temp'.")
    var id: String
        ...
        do {
            ...
            if options.json {
                ...
                if !noLogs {
                    ...
                    let logs = try client(options).get("logs?server=\(escaped)&tail=\(count)", as: MarinaAPI.LogsResponse.self)
...
struct AddProject: ParsableCommand {
    ...
    func run() throws {
        ...
        let body = MarinaAPI.AddProjectRequest(
            name: name,
            root: path,
            icon: icon,
            color: color,
...
struct MemoryLimit: ParsableCommand {
    ...
    func run() throws {
        ...
        let body = MarinaAPI.UpdateMemoryLimitRequest(
            project: project,
            mode: selection.mode,
            bytes: selection.bytes
        )
        do {
            let response = try client(options).post("memory-limit", body, as: MarinaAPI.ActionResponse.self)
            emit(response, json: options.json) { $0.message }
        } catch {
            fail(error.localizedDescription)
        }
...
private func renderMemoryLimits(_ status: MarinaStatus) -> String {
...
struct AddServer: ParsableCommand {
    ...
    func run() throws {
        ...
        let body = MarinaAPI.AddServerRequest(
...
struct UpdateServer: ParsableCommand {
    ...
    func run() throws {
        ...
        var body = MarinaAPI.UpdateServerRequest(server: server)
        body.name = name
...
struct Remove: ParsableCommand {
    ...
    func run() throws {
        do {
            if let project {
                let body = MarinaAPI.RemoveRequest(project: project)
                let response = try client(options).post("projects/remove", body, as: MarinaAPI.ActionResponse.self)
                emit(response, json: options.json) { $0.message }
            } else if let server {
                let body = MarinaAPI.RemoveRequest(server: server)
                let response = try client(options).post("servers/remove", body, as: MarinaAPI.ActionResponse.self)
                emit(response, json: options.json) { $0.message }
            } else {
                fail("Pass a server name, or --project <name>.")
            }
...
struct TakeOver: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        abstract: "Stop an external listener and relaunch the configured server under Marina.",
        aliases: ["adopt"]
    )
    ...
    func run() throws {
        do {
            let body = MarinaAPI.TakeOverRequest(server: server)
            let response = try client(options).post("servers/take-over", body, as: MarinaAPI.ActionResponse.self)
            emit(response, json: options.json) { $0.message }
        } catch {
            fail(error.localizedDescription)
        }
...
struct Port: ParsableCommand {
    ...
    func run() throws {
        do {
            let response = try client(options).get("ports?port=\(port)", as: MarinaAPI.PortQueryResponse.self)
            emit(response, json: options.json) { result in
                guard let occupant = result.occupant else { return "Port \(result.port) is free." }
                let owner = occupant.ownedByMarina ? " (managed by Marina)" : ""
...
struct KillPort: ParsableCommand {
    ...
    func run() throws {
        do {
            let response = try client(options).post("ports/kill", MarinaAPI.KillPortRequest(port: port), as: MarinaAPI.ActionResponse.self)
            emit(response, json: options.json) { $0.message }
        } catch {
            fail(error.localizedDescription)
        }
...
struct Open: ParsableCommand {
    static let configuration = CommandConfiguration(abstract: "Bring the Marina window to the front.")
    ...
    func run() throws {
        ...
        do {
            ...
            let response = try client(options).post(
                "open",
                MarinaAPI.OpenRequest(destination: destination),
                as: MarinaAPI.ActionResponse.self
            )
            emit(response, json: options.json) { $0.message }
        } catch {
            fail(error.localizedDescription)
...
struct Quit: ParsableCommand {
    static let configuration = CommandConfiguration(abstract: "Quit Marina, stopping every server.")
    @OptionGroup var options: GlobalOptions

    func run() throws {
        do {
            let response = try client(options).request(
                "POST", "quit", body: MarinaAPI.Empty(), as: MarinaAPI.ActionResponse.self, autoLaunch: false
            )
            emit(response, json: options.json) { $0.message }
        } catch {
            fail(error.localizedDescription)
...
private enum ForeverManager {
    static let label = "dev.marina.forever"
    ...
    static let appExecutable = "/Applications/Marina.app/Contents/MacOS/Marina"
    ...
    static func enable(options: GlobalOptions) throws -> ForeverState {
        guard FileManager.default.isExecutableFile(atPath: appExecutable) else {
            throw ValidationError("Install /Applications/Marina.app first with ./build.sh --run.")
        }
        ...
        try fm.createDirectory(at: MarinaPaths.logsDirectory, withIntermediateDirectories: true)
        ...
        let plist: [String: Any] = [
            ...
            "StandardOutPath": MarinaPaths.logsDirectory.appendingPathComponent("marina-launchd.log").path,
            "StandardErrorPath": MarinaPaths.logsDirectory.appendingPathComponent("marina-launchd-error.log").path,
        ]
        ...
        guard waitForAPI(options: options) else {
            throw ValidationError("launchd loaded Marina, but its control API did not become reachable.")
        }
    ...
    static func disable(options: GlobalOptions) throws -> ForeverState {
        ...
        if !activeServers.isEmpty {
            ...
            guard c.launchAppIfNeeded(), waitForAPI(options: options) else {
                throw ValidationError("Launch at login was disabled, but Marina could not be relaunched.")
            }
            try restore(activeServers, options: options)
        }
        return state()
    ...
    private static func stopCurrentApp(options: GlobalOptions) throws -> [String] {
        ...
        if c.isReachable() {
            let status = try c.get("status", as: MarinaStatus.self)
            active = status.projects.flatMap(\.servers).filter {
                $0.state != .stopped && $0.state != .failed
            }.map(\.id)

            _ = try c.request(
                "POST", "quit", body: MarinaAPI.Empty(),
                as: MarinaAPI.ActionResponse.self, autoLaunch: false
            )
    ...
    private static func terminateOtherAppInstances() {
        ...
        process.arguments = [
            "-U", String(getuid()), "-f",
            #"/Marina\.app/Contents/MacOS/Marina$"#,
        ]
    ...
    private static func restore(_ serverIDs: [String], options: GlobalOptions) throws {
        let c = client(options)
        for id in serverIDs {
            let body = MarinaAPI.TargetRequest(server: id)
            _ = try c.post("start", body, as: MarinaAPI.ActionResponse.self)
        }
    }
...
struct Forever: ParsableCommand {
    static let configuration = CommandConfiguration(
        abstract: "Keep Marina available across macOS logins.",
        subcommands: [Enable.self, Disable.self, ForeverStatus.self],
        defaultSubcommand: ForeverStatus.self
    )

    struct Enable: ParsableCommand {
        static let configuration = CommandConfiguration(abstract: "Install and load the Marina LaunchAgent.")
        @OptionGroup var options: GlobalOptions

        func run() throws {
            let state = try ForeverManager.enable(options: options)
            emit(state, json: options.json) { _ in
                "Marina will launch at login and is now supervised by launchd."
            }
        }
    }

    struct Disable: ParsableCommand {
        static let configuration = CommandConfiguration(abstract: "Unload the Marina LaunchAgent.")
        @OptionGroup var options: GlobalOptions

        func run() throws {
            let state = try ForeverManager.disable(options: options)
            emit(state, json: options.json) { _ in "Marina launch at login is disabled." }
        }
    }
...
struct Config: ParsableCommand {
    ...
    func run() throws {
        let url = MarinaPaths.configFile
        if pathOnly {
            print(url.path)
            return
        }
        ...
        if let data = try? Data(contentsOf: url) {
            ...
            print("(not created yet, launch Marina once)")
        }
    }
}
