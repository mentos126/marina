...
// MARK: - Config model (what lives in ~/.config/marina/config.json)
...
public struct MarinaConfig: Codable {
    ...
    public init(
        version: Int = 1,
        apiPort: Int = MarinaConfig.defaultAPIPort,
    ...
    public init(from decoder: Decoder) throws {
        ...
        apiPort = try c.decodeIfPresent(Int.self, forKey: .apiPort) ?? MarinaConfig.defaultAPIPort
...
public struct MarinaStatus: Codable {
...
public struct PortOccupant: Codable, Hashable {
    ...
    /// True when the listener is one of Marina's own supervised servers.
    public var ownedByMarina: Bool
    public var serverID: String?
    /// Docker Desktop owns the host listener for published container ports.
    /// These fields identify the actual container Marina can stop safely.
    ...
    public init(
        ...
        self.command = command
        self.user = user
        self.ownedByMarina = ownedByMarina
